document.addEventListener("DOMContentLoaded", function () {
  const contenedorPedidoAlProveedor = document.getElementById(
    "contenedorPedidoAlProveedor"
  );

  const ingresoManual = document.createElement("textarea");
  ...
  ingresoManual.id = "pedidoAlProveedor";
  ...
  botonCrearMensaje.id = "unirMensajes";
...
/*     let mensajeArvhico = ingresoArchivo.value; */
    ...
    if (mensajeManual.trim() === "" && mensajeArvhico.length === 0) {
      Swal.fire(
        "Por favor, escribí un mensaje o carga un archivo antes de enviarlo."
      );
      return;
    }

/*     if (mensajeArvhico.length > 0) {
      //explicame toto porque lloro no entender
  ...
  contenedorPedidoAlProveedor.append(
    ingresoManual,
/*     ingresoArchivo,
    botonArvhivo, */
    botonCrearMensaje
